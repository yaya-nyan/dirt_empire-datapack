...
execute as @e[type=item_frame,tag=decoration] at @s if block ~ ~-1 ~ air run summon item ~ ~ ~ {Item:{Count:1,id:"minecraft:knowledge_book"}}
...
execute as @e[type=item_frame,tag=decoration] at @s unless block ~ ~ ~ air run summon item ~ ~ ~ {Item:{Count:1,id:"minecraft:knowledge_book"}}
