...
execute as @e[type=item_frame,tag=decoration] at @s unless block ~ ~ ~ air run summon item ~ ~ ~ {Item:{Count:1,id:"minecraft:item_frame"},Motion:[0.0,0.25,0.0]}
...
execute as @e[type=item_frame,tag=decoration,name=christmas_tree] at @s unless block ~ ~ ~ air run function dirtempire:items/convert_to/christmas_tree
